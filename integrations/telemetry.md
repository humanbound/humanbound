...
??? question "How do I enable telemetry?"
    Add a `telemetry` block to your agent config JSON alongside `chat_completion` and `thread_init`. If the block is present, telemetry is enabled — no separate flag is needed. Specify the observability platform `format`, the `endpoint` URL, and auth headers.
