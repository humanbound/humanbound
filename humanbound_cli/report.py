...
def _severity_to_label(severity):
    ...
    elif severity >= 50: return "high"
    elif severity >= 25: return "medium"
    elif severity >= 1: return "low"
    return "info"
...
def generate_html_report(experiment, logs):
    ...
    posture_grade = posture_data.get("grade", _score_to_grade(posture_score)) if posture_data else "F"
    ...
    if insights:
        ...
        if fail_insights:
            ...
            for ins in sorted(fail_insights, key=lambda x: SEVERITY_ORDER.index(_severity_to_label(x.get("severity", 0))) if _severity_to_label(x.get("severity", 0)) in SEVERITY_ORDER else 99):
                sev = _severity_to_label(ins.get("severity", 0))
    ...
    if fail_logs:
        ...
        for i, log in enumerate(fail_logs[:20], 1):
            ...
            body += f'<div style="margin-bottom:20px; padding:16px; border:1px solid #e2e8f0; border-radius:8px;">\n'
            body += f'<div style="display:flex; gap:8px; align-items:center; margin-bottom:8px;">'
            body += f'<span class="badge badge-{sev}">{sev}</span> '
            body += f'<strong>{cat}</strong></div>\n'
            body += f'<p style="font-size:13px; color:#475569; margin-bottom:8px;">{explanation}</p>\n'
