---
description: "Export guardrails from your test results — rule lists for gateways and moderation APIs, or the humanbound-firewall policy file (agent.yaml)."
keywords:
  - guardrails export
...
- **Rule lists** for the enforcement points that consume them — gateways, moderation APIs, your own filters: the attack patterns a local run found, as JSON, or an OpenAI Guardrails configuration (`--vendor openai`) when logged in.
