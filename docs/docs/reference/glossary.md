...
| Term | Definition |
|------|-----------|
| **AI SecOps** | The AI equivalent of DevSecOps — a repeating cycle of testing, monitoring, and protecting AI systems in production. Includes vulnerability detection through automated probing, model improvement through feedback loops, and runtime protection through the firewall. The goal is continuous assurance, not one-off audits. See [AI SecOps concept page](../concepts/ai-secops.md). |
| **AI TDD** | The IDE surface of [AI SecOps](../concepts/ai-secops.md): test-driven definition of an AI agent's security boundaries inside the editor (Claude Code, Cursor) via the [Humanbound plugins](../plugins/index.md). The "last mile" — rules get set while you code, before anything ships. |
