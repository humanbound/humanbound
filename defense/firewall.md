...
```python
from hb_firewall import [Provider](../reference/glossary.md#architecture-distribution), ProviderIntegration, ProviderName
...
??? question "How is the Tier 2 classifier trained?"
    Tier 2 is trained from your Humanbound adversarial and QA test logs — failed adversarial conversations supply attack examples and passed QA conversations supply benign examples. Run `hb firewall train` after accumulating test data to produce a `.hbfw` model file.
